          ...
          <div className="px-0 md:px-9 py-0">
            <ul>
              {notificationsData.map((notification, idx) => {
                const { sender, priority, createdAt, description, read } =
                  notification;
                return (
                  <li
                    key={idx}
        ...
        <div className="h-screen bg-background w-full max-w-full box-border p-6 md:pr-9">
          ...
          <div className="flex justify-center items-center h-[80vh]">
            <div className="flex justify-center flex-col items-center h-[60vh] md:h-[80vh] lg:h-[75vh] 2xl:h-[80vh]">
              <EmptyNotification className="scale-[0.6] sm:scale-75 lg:scale-90 notificationSvg" />
              <h1 className="text-2xl sm:text-3xl 2xl:text-5xl font-normal">
                No Notifications
              </h1>
              <p className="my-3">
                click here to see{" "}
                <Link to="/issues" className="underline decoration-solid">
                  issues.
                </Link>
              </p>
              <button className="flex mx-auto justify-center items-center rounded-full font-medium text-xs sm:text-sm lg:text-md bg-accenture h-14 w-32 sm:w-40 sm:h-12">
                Create Notification
              </button>
            </div>
          </div>
...
function showPriorityLevel(priority: string, read: boolean) {
  if (priority === "low") {
    return (
      <span
        className={`bg-[#4BB543] w-3 h-3 rounded-full ${
    ...
    return (
      <span
        className={`bg-[#FFD601] w-3 h-3 rounded-full ${
          read ? "opacity-50" : "opacity-100"
        }`}
      />
    );
    ...
    return (
      <span
        className={`bg-[#CE2029] w-3 h-3 rounded-full ${
          read ? "opacity-50" : "opacity-100"
        }`}
      />
    );
